...
<head>
        ...
        <!-- Custom CSS -->
        <link href="{{ asset('/forntend/assets/css/plugins/animation.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/bootstrap.min.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/flaticon.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/font-awesome.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/iconfont.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/ion.rangeSlider.min.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/light-box.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/line-icons.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/slick-theme.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/slick.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/snackbar.min.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/plugins/themify.css')}}" rel="stylesheet">
        <link href="{{ asset('/forntend/assets/css/styles.css')}}" rel="stylesheet">
    ...
    <body>
       ...
       <div class="preloader"></div>
        ...
        <div id="main-wrapper">
						...
						<div class="col-xl-5 col-lg-6 col-md-12 col-sm-12">
							...
							<div class="language-selector-wrapper dropdown js-dropdown float-right mr-3">
								...
								<ul class="dropdown-menu popup-content link">
									...
									<li><a href="javascript:void(0);" class="dropdown-item medium text-muted"><img src="{{ asset('/forntend/assets/img/2.jpg')}}" alt="fr" width="16" height="11" /><span>Français</span></a></li>
								</ul>
							</div>
							
							<div class="currency-selector dropdown js-dropdown float-right mr-3">
								@auth('customerlogin')
									{{--  <a href="" class="text-muted medium"><i class="lni lni-user mr-1"></i>
										{{ Auth::guard('customerlogin')->user()->name }}</a>   --}}
										<a class="popup-title" href="javascript:void(0)" data-toggle="dropdown" title="Language" aria-label="Language dropdown"><i class="lni lni-user mr-1"></i>
											{{ Auth::guard('customerlogin')->user()->name }}
										</a>
										<ul class="dropdown-menu popup-content link">
											<li class="current">
												<a href="{{ route('customer.profile') }}" class="dropdown-item medium text-muted">
													<i class="lni lni-user mr-1"></i>
													<span>My Profile</span>
												</a>
											</li>
											<li>
												<a href="{{ route('customer.logout') }}" class="dropdown-item medium text-muted">
													<i class="lni lni-user mr-1"></i>
													<span>Logout</span>
												</a>
											</li>
								...
								@else
								<a href="{{ route('customer.login') }}" class="text-muted medium">
									<i class="lni lni-user mr-1"></i>
									Sign In
								</a> <span>/</span> 
								...
								<div class="headd-sty-left d-flex align-items-center">
									<div class="headd-sty-01">
										<a class="nav-brand py-0" href="#">
											<img src="{{ asset('/forntend/assets/img/logo.png')}}" class="logo" alt="" />
										</a>
									</div>
			...
			<div class="w3-ch-sideBar w3-bar-block w3-card-2 w3-animate-right" style="display:none;right:;" id="Cart">
					...
					<div class="right-ch-sideBar">
						...
						<div class="cart_select_items py-2">
							...
							@php
								$subtotal = 0;
							@endphp
							@forelse (App\Models\Cart::where('customer_id',Auth::guard('customerlogin')->user()->id)->get() as $cart )
								<div class="d-flex align-items-center justify-content-between br-bottom px-3 py-3">
									<div class="cart_single d-flex align-items-center">
										<div class="cart_selected_single_thumb">
											<a href="#"><img src="assets/img/product/4.jpg" width="60" class="img-fluid" alt="" /></a>
										...
										<div class="cart_single_caption pl-2">
											<h4 class="product_title fs-sm ft-medium mb-0 lh-1">{{ $cart->rel_to_product->product_title }}</h4>
											<p class="mb-2">
												<span class="text-dark ft-medium small">{{ $cart->rel_to_size->size_name}}</span>
												, <span class="text-dark small">{{ $cart->rel_to_color->color_name}}</span>
												<span class="text-dark small">{{ $cart->quantity}}</span>
											</p>
											<h4 class="fs-md ft-medium mb-0 lh-1">BDT {{ $cart->rel_to_product->after_discount*$cart->quantity}}</h4>
										</div>
									</div>
									<div class="fls_last">
										<a href="{{ route('cart.delete',$cart->id) }}"><button class="close_slide gray"><i class="ti-close"></i></button></a>
									</div>
								</div>
								@php
								...
								@empty
								<div class="align-items-center justify-content-between br-top br-bottom px-3 py-3">
									<p>
										No Product Yet!!!
								...
								<div class="d-flex align-items-center justify-content-between br-top br-bottom px-3 py-3">
									...
									<h3 class="mb-0 ft-medium">BDT {{ $subtotal }}</h3>
								</div>
								<div class="cart_action px-3 py-3">
									<div class="form-group">
										<a href="{{ route('cart.page') }}"><button type="button" class="btn d-block full-width btn-dark-light">View Cart</button></a>
										<a href="{{ route('checkout') }}"><button type="button" class="btn d-block mt-2 full-width btn-dark-light">Procceed To Checkout</button></a>
									</div>
								</div>	
						</div>	
					</div>
		...
		<script src="{{ asset('/forntend/assets/js/ion.rangeSlider.min.js') }}"></script>
		<script src="{{ asset('/forntend/assets/js/slick.js') }}"></script>
		<script src="{{ asset('/forntend/assets/js/slider-bg.js') }}"></script>
		<script src="{{ asset('/forntend/assets/js/lightbox.js') }}"></script> 
		<script src="{{ asset('/forntend/assets/js/smoothproducts.js') }}"></script>
		<script src="{{ asset('/forntend/assets/js/snackbar.min.js') }}"></script>
		<script src="{{ asset('/forntend/assets/js/jQuery.style.switcher.js') }}"></script>
		<script src="{{ asset('/forntend/assets/js/custom.js') }}"></script>
		<script src="//cdn.jsdelivr.net/npm/sweetalert2@11"></script>
		<script src="https://use.fontawesome.com/8845182205.js"></script>
		<!-- ============================================================== -->
		<!-- This page plugins -->
		<!-- ============================================================== -->	
        @yield('footerscript')
